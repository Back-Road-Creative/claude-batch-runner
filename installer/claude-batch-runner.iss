; Inno Setup script for the claude-batch-runner Windows installer.
;
; Expects PyInstaller to have already written dist\claude-batch-runner.exe, and
; takes the version on the command line. There is deliberately no default, so a
; build that forgets it fails loudly rather than shipping a wrong version number:
;
...
; Every unit of work is a `claude -p` subprocess (claude_batch_runner/driver.py),
; so without the Claude Code CLI on PATH this tool cannot run a campaign at all.
; This installer does not bundle it: it is a separately versioned, self-updating
; product with its own installer. Said on the way out, not buried.
#define ClaudeNote "claude-batch-runner runs the Claude Code CLI (claude). It is NOT included and must be installed separately, on PATH:%n%n    winget install Anthropic.ClaudeCode%n%nThen run `claude` once to sign in. Open a new terminal, then run:  claude-batch-runner --help"
...
AppId={{04F25EFE-CC97-4DCD-B83E-D24A1B070041}
AppName={#AppName}
...
FinishedLabel=Setup has installed [name] on your computer.%n%n{#ClaudeNote}
FinishedLabelNoIcons=Setup has installed [name] on your computer.%n%n{#ClaudeNote}
